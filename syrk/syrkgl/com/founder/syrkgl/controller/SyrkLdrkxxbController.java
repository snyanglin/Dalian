...
/******************************************************************************
 * @Package: [com.founder.syrkgl.controller.SyrkLdrkxxbController.java]
 * @ClassName: [SyrkLdrkxxbController]
 * @Description: [流动人口信息表]
 * @Author: [hongguanjun]
 * @CreateDate: [2015-4-14 下午7:29:42]
 * @UpdateUser: [hongguanjun(如多次修改保留历史记录，增加修改记录)]
 * @UpdateDate: [2015-4-14 下午7:29:42，(如多次修改保留历史记录，增加修改记录)]
 * @UpdateRemark: [说明本次修改内容,(如多次修改保留历史记录，增加修改记录)]
 * @Version: [v1.0]
 */
@Controller
@RequestMapping(value = "syrkLdrkxxb")
public class SyrkLdrkxxbController extends BaseController {

	private Logger logger = Logger.getLogger(this.getClass());

	@Resource(name = "syrkLdrkxxbService")
	private SyrkLdrkxxbService syrkLdrkxxbService;
	...
	public ModelAndView edit(@PathVariable(value = "id") String id,
		...
		if (sessionBean != null
			...
			SyrkLdrkxxb ldrk = syrkLdrkxxbService.queryById(id);
			if (ldrk == null) {
				throw new BussinessException("查询无数据！");
			}
			mv.addObject(AppConst.MESSAGES, new Gson().toJson(ldrk));
			return mv;
		} else {
			ModelAndView mv = new ModelAndView("syrkgl/syrkLdrkxxb");
			SyrkLdrkxxb ldrk = syrkLdrkxxbService.queryById(id);
			if (ldrk == null) {
				throw new BussinessException("查询无数据！");
			}
	...
	@RequestMapping(value = "/{id}", method = { RequestMethod.POST,
			RequestMethod.PUT })
	public ModelAndView update(SyrkLdrkxxb entity, SessionBean sessionBean) {
		ModelAndView mv = new ModelAndView(getViewName(sessionBean));
		Map<String, Object> map = new HashMap<String, Object>();
		sessionBean = getSessionBean(sessionBean);
		try {
			syrkLdrkxxbService.update(entity, sessionBean);
