...
/******************************************************************************
 * @Package: [com.founder.syrkgl.controller.SyrkWlhryxxbController.java]
 * @ClassName: [SyrkWlhryxxbController]
 * @Description: [未落户人员信息表]
 * @Author: [hongguanjun]
 * @CreateDate: [2015-4-14 下午7:30:01]
 * @UpdateUser: [hongguanjun(如多次修改保留历史记录，增加修改记录)]
 * @UpdateDate: [2015-4-14 下午7:30:01，(如多次修改保留历史记录，增加修改记录)]
 * @UpdateRemark: [说明本次修改内容,(如多次修改保留历史记录，增加修改记录)]
 * @Version: [v1.0]
 */
@Controller
@RequestMapping(value = "syrkWlhryxxb")
public class SyrkWlhryxxbController extends BaseController {

	private Logger logger = Logger.getLogger(this.getClass());

	@Resource(name = "syrkWlhryxxbService")
	private SyrkWlhryxxbService syrkWlhryxxbService;
	@Resource
	...
	public ModelAndView edit(@PathVariable(value = "id") String id,
		...
		if (sessionBean != null
			...
			SyrkWlhryxxb wlrk = syrkWlhryxxbService.queryById(id);
			if (wlrk == null) {
				throw new BussinessException("查询无数据！");
			}
			mv.addObject(AppConst.MESSAGES, new Gson().toJson(wlrk));
			return mv;
		} else {
			ModelAndView mv = new ModelAndView("syrkgl/syrkWlhryxxb");
			SyrkWlhryxxb wlrk = syrkWlhryxxbService.queryById(id);
			if (wlrk == null) {
				throw new BussinessException("查询无数据！");
			}
	...
	@RequestMapping(value = "/{id}", method = { RequestMethod.POST,
			RequestMethod.PUT })
	public ModelAndView update(SyrkWlhryxxb entity, SessionBean sessionBean) {
		ModelAndView mv = new ModelAndView(getViewName(sessionBean));
		Map<String, Object> map = new HashMap<String, Object>();
		sessionBean = getSessionBean(sessionBean);
		try {
			syrkWlhryxxbService.update(entity, sessionBean);
