...
/******************************************************************************
 * @Package: [com.founder.syrkgl.controller.SyrkCzrkxxbController.java]
 * @ClassName: [SyrkCzrkxxbController]
 * @Description: [常住人口信息表]
 * @Author: [hongguanjun]
 * @CreateDate: [2015-4-14 下午7:28:37]
 * @UpdateUser: [hongguanjun(如多次修改保留历史记录，增加修改记录)]
 * @UpdateDate: [2015-4-14 下午7:28:37，(如多次修改保留历史记录，增加修改记录)]
 * @UpdateRemark: [说明本次修改内容,(如多次修改保留历史记录，增加修改记录)]
 * @Version: [v1.0]
 */
@Controller
@RequestMapping(value = "syrkCzrkxxb")
public class SyrkCzrkxxbController extends BaseController {

	private Logger logger = Logger.getLogger(this.getClass());

	@Resource(name = "syrkCzrkxxbService")
	private SyrkCzrkxxbService syrkCzrkxxbService;
	...
	public ModelAndView edit(@PathVariable(value = "id") String id,
		...
		if (sessionBean != null
			...
			SyrkCzrkxxb czrk = syrkCzrkxxbService.queryById(id);
			if (czrk == null) {
				throw new BussinessException("查询无数据！");
			}
			mv.addObject(AppConst.MESSAGES, new Gson().toJson(czrk));
			return mv;
		} else {
			ModelAndView mv = new ModelAndView("syrkgl/syrkCzrkxxb");
			SyrkCzrkxxb czrk = syrkCzrkxxbService.queryById(id);
			if (czrk == null) {
				throw new BussinessException("查询无数据！");
			}
	...
	@RequestMapping(value = "/{id}", method = { RequestMethod.POST,
			RequestMethod.PUT })
	public ModelAndView update(SyrkCzrkxxb entity, SessionBean sessionBean) {
		ModelAndView mv = new ModelAndView(getViewName(sessionBean));
		Map<String, Object> map = new HashMap<String, Object>();
		sessionBean = getSessionBean(sessionBean);
		try {
			syrkCzrkxxbService.update(entity, sessionBean);
