...
public class DictTree_DL_D_FZXZDM extends DictTreeCreator {

	private String treeName = "问题性质代码";

	public DictTree_DL_D_FZXZDM() {
	}
	...
	public void createTreeJS(String zdbh, String zdbhMc) throws Exception {
		...
		if (dictList != null && dictList.size() > 0) { // 有数据
			...
			for (int i1 = 0; i1 < dictList.size(); i1++) {
				...
				if (dm1.endsWith("0000")&&dm1.length()==6) { // 第1级判断条件
					...
					String dm1Matching = dm1.substring(0, 2); // 下一级为前1位相同
