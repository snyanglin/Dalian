...
public class DictTree_XZ_D_SAWPFLHDM extends DictTreeCreator {

	private String treeName = "作案工具"; // 字典名称

	public DictTree_XZ_D_SAWPFLHDM() {
	}
			...
			for (int i1 = 0; i1 < dictList.size(); i1++) {
				...
				if (dm1.length()==1) { // 第1级判断条件
					...
					for (int i2 = 0; i2 < dictList.size(); i2++) {
						...
						if (!dm2.equals(dm1) && dm2.startsWith(dm1)
							...
							String dm2Matching = dm2.substring(0, 3); // 下一级为前2位相同
