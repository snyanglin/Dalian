...
public class DictTree_D_CL_CLLX extends DictTreeCreator {

	private String treeName = "车辆类型树"; // 字典名称

	public DictTree_D_CL_CLLX() {
	}
			...
			for (int i1 = 0; i1 < dictList.size(); i1++) {
				...
				if (dm1.length()==1) { // 第1级判断条件
					...
					for (int i2 = 0; i2 < dictList.size(); i2++) {
						...
						if (!dm2.equals(dm1) && dm2.startsWith(dm1)) { // 第2级判断条件
