...
public class ZdryApprovalController extends BaseController {
	...
	public @ResponseBody
		...
		ZdryZb zdryZdryzb = zdryZdryzbService.queryById((String)workflowXx.get("zdryId"));
	...
	public @ResponseBody
		Map getApprovalOrgCode(String condition) {
		Map<String, Object> model = new HashMap<String, Object>();
		...
		//从本部门上行指定等级代码的部门
	//	OrgOrganization orgOrganization1=orgOrganizationService.queryUpOrgByLevel(orgcode,"");
		// 取得两个部门的共同上级部门
	//    OrgOrganization orgOrganization2=orgOrganizationService.querySameParentOrg(orgcode,"");
	...
	//	List<HistoricDetail> aa=ahistoryService.createHistoricDetailQuery().formProperties().list();
		
		Object renderedStartForm = formService.getRenderedStartForm(entity.getPocessDefinitionId());
//		Object renderedStartForm1 =formService.getTaskFormData(entity.getWorkflowId());
//		Object renderedTsskForm=formService.getRenderedTaskForm(entity.getWorkflowId(),"approval.form");
	...
	EasyUIPage queryList(EasyUIPage page,
			...
			if("1".equals(splxString)){
				...
				List<JExecution> executions  =(List<JExecution>) runtimeService.findJExecutionsByUser(sessionBean, page.getBegin(),rows , filrers).get(JExecution.EXECUTIONLIST);
				...
				for (int i = 0; i < executions.size(); i++) {
					...
					String zdrylx =(String)processDefinitionService.getVariables(execution.getId()).get("zdrylx");
					String sqrId=(String)processDefinitionService.getVariables(execution.getId()).get("applyUserId");//申请人ID
				    String createTime=(String)processDefinitionService.getVariables(execution.getId()).get("createTime");
					String sqlx=(String)processDefinitionService.getVariables(execution.getId()).get("sqlx");//申请类型
					String sqlxdm=(String)processDefinitionService.getVariables(execution.getId()).get("sqlxdm");//申请类型
					String zdryId=(String)processDefinitionService.getVariables(execution.getId()).get("zdryId");//人员USERID
					String zdryName=(String)processDefinitionService.getVariables(execution.getId()).get("xm");//人员USERID
					String sqrName=(String)processDefinitionService.getVariables(execution.getId()).get("sqrName");//申请人姓名
					
				
					zdryWorkflowVO.setPocessDefinitionId(execution.getProcessDefinitionId());
				...
				page.setTotal((int) runtimeService.findJExecutionsByUser(sessionBean, page.getBegin(),rows , filrers).get(JExecution.EXECUTIONTOTALCOUNT));
			...
			if("2".equals(splxString)){
				...
				List<JHistoricTaskInstance> historicTaskInstances=  (List<JHistoricTaskInstance>) historyService.findJFinishedTasksByUser(sessionBean, page.getBegin(),rows , filrers).get(JHistoricTaskInstance.HistoricTaskInstanceLIST);
			
				List<ZdryWorkflowVO> zdryWorkflowList= new ArrayList();
				for (int i = 0; i < historicTaskInstances.size(); i++) {
					ZdryWorkflowVO zdryWorkflowVO=new ZdryWorkflowVO();
					JHistoricTaskInstance historicTaskInstance = historicTaskInstances.get(i);
				...
				//	ahistoryService.createHistoricTaskInstanceQuery().processDefinitionId(historicTaskInstance.getProcessInstanceId()).singleResult().getId();
					...
					String zdrylx =(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("zdrylx");
					String sqrId=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("applyUserId");//申请人ID
				    String createTime=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("createTime");
					String sqlx=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("sqlx");//申请类型
					String sqlxdm=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("sqlxdm");//申请类型
					String zdryId=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("zdryId");//人员USERID
					String zdryName=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("xm");//人员USERID
					String sqrName=(String)historyService.getJHistoricVariable(historicTaskInstance.getProcessInstanceId()).get("sqrName");//申请人姓名
					
				
					zdryWorkflowVO.setWorkflowId(historicTaskInstance.getId());
					zdryWorkflowVO.setPocessDefinitionId(historicTaskInstance.getProcessDefinitionId());
					zdryWorkflowVO.setZdrylx(zdrylx);
					zdryWorkflowVO.setSqrId(sqrId);
					zdryWorkflowVO.setCreateTime(createTime);
					zdryWorkflowVO.setSqlx(sqlx);
				...
				page.setTotal((int) historyService.findJFinishedTasksByUser(sessionBean, page.getBegin(),rows , filrers).get(JHistoricTaskInstance.HistoricTaskInstanceTOTALCOUNT));
				page.setRows(zdryWorkflowList);
			}
		
			if("0".equals(splxString)||splxString==null||splxString.equals("")){
				List<JTask> tasks = (List<JTask>) taskService.findJTasksByUser(sessionBean, page.getBegin(),rows , filrers).get(JTask.TASKLIST);
				
				List<ZdryWorkflowVO> zdryWorkflowList= new ArrayList();
				for (int i = 0; i < tasks.size(); i++) {
					ZdryWorkflowVO zdryWorkflowVO=new ZdryWorkflowVO();
					JTask task = tasks.get(i);
					Map<String, Object>  vars=processDefinitionService.getVariables(task.getExecutionId());
					...
					zdryWorkflowVO.setPocessDefinitionId(task.getProcessDefinitionId());
					zdryWorkflowVO.setZdrylx(zdrylx);
					zdryWorkflowVO.setSqrId(sqrId);
					zdryWorkflowVO.setCreateTime(createTime);
					zdryWorkflowVO.setSqlx(sqlx);
				...
				page.setTotal((int) taskService.findJTasksByUser(sessionBean, page.getBegin(),rows , filrers).get(JTask.TASKTOTALCOUNT));
				page.setRows(zdryWorkflowList);
			}
	...
	public ModelAndView zdryApproval(ZdryWorkflowVO zdryWorkflowVO,SessionBean sessionBean) {
		...
		if(!(zdryWorkflowVO.getNextSpUserId().equals("")||zdryWorkflowVO.getNextSpUserId()==null)){
			
			//	variables.put(zdryWorkflowVO.getNextSppos()+"Approved", zdryWorkflowVO.getNextSpUserId());
				variables.put("fxjzg", zdryWorkflowVO.getNextSpUserId());
				
				
		}
		if((zdryWorkflowVO.getNextSpUserId().equals("")||zdryWorkflowVO.getNextSpUserId()==null)
				&&(!(zdryWorkflowVO.getNextSpposId().equals("")||zdryWorkflowVO.getNextSpposId()==null))){
			
			//	variables.put(zdryWorkflowVO.getNextSppos()+"Approved", zdryWorkflowVO.getNextSpOrgCode()+"_"+zdryWorkflowVO.getNextSpposId());
				variables.put("fxjzg", zdryWorkflowVO.getNextSpOrgCode()+"_"+zdryWorkflowVO.getNextSpposId());	
		}
		
		String Key=taskService.findJTaskbyTaskId(zdryWorkflowVO.getWorkflowId()).getTaskDefinitionKey();
	//	variables.put(taskService.findJTaskbyTaskId(zdryWorkflowVO.getWorkflowId()).getTaskDefinitionKey()+"ApprovedType", zdryWorkflowVO.getSpjg());
		
		variables.put("sz"+"ApprovedType", zdryWorkflowVO.getSpjg());

	//	variables.put(orgPositionService.queryByPosid(sessionBean.getUserPositionId()).getId()+"ApprovedType", zdryWorkflowVO.getSpjg());

		Zdry zdryZdryzb = zdryZdryzbService.getZdry();
		...
		if(zdryWorkflowVO.getSpjg().equals("0")){//拒绝
			...
			variables.put("szApprovedType", "1");	
		        ...
		        if(zdryWorkflowVO.getNextSpposId()!=null){
		        	taskParameter=zdryWorkflowVO.getNextSpOrgCode()+"_"+zdryWorkflowVO.getNextSpposId();
		        }
...
//			taskService.claimTask(zdryWorkflowVO.getWorkflowId(), sessionBean.getUserId());  //接收任务
	...
	public JsonObject getUserByOrgAndPos(@PathVariable String orgcode,@PathVariable String posid){
		JsonObject jObject = new JsonObject();
		OrgAssignPublic orgAssignPublic = new OrgAssignPublic();
		List<OrgUserInfo> users = orgAssignPublic.queryUserByOrgAndPos(orgcode, posid);
