 ...
 */
@Service()
public class XsfctxService extends BaseDock implements Docking {
	...
	public Map<String, Object> main(HttpServletRequest request) {
		...
		list = xsjbxxbDao.queryFCById();
		List<String> exr = new ArrayList<String>();
		try {
			for (Xsjbxxb item : list) {
				if(nowDayDiff(item.getFcsj()) >= diff){
					...
					for (OrgUser orgUser : list2) {
						...
						message.setYwurl("/xsjbxxb/openFC?id="+item.getId()+"&mainTabID=0");		//业务URL
						...
						message.setXxnr("【有奖举报】，【"+commonService.queryNameOrgID(item.getSlfzdm())+"】有一条线索【"+item.getId()+"】需要进行30日反馈处理，已逾期"+nowDayDiff(item.getFcsj())+"天");	//消息内容
						...
						message.setXxbt("线索30日反馈处理提醒");		//信息标题
						message.setDkfs("0");					//打开方式
						this.sysMessageDao.saveMessage(message);
						exr.add("向部门["+item.getSlfzdm()+"]发送消息提醒（有一条线索["+item.getId()+"]需要进行30日反馈处理）");
					}
				}
