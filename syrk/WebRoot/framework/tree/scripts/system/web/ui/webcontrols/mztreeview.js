...
MzTreeView.icons = 
{
  ...
  collapse:
  {
    ...
    "ajaxWaiting":"ajaxWaiting.gif",
    "user":"user.gif",
    "workgroup":"workgroup.gif",
...
MzTreeView.prototype.contextMenuHandle = function(e)
{
  ...
  if((e.tagName=="A" || e.tagName=="IMG")&& e.id)
  {
     ...
     e.focus();
	 var contextMethod=e.get("contextMethod");
	 if (("undefined" != typeof contextMethod) && (contextMethod != "")) {
		 eval(contextMethod);
	 }
    this.currentNode=e;
	 this.dispatchEvent(new System.Event("oncontextmenu"));
	 oldE.returnValue = false;
  }
};
MzTreeView.prototype.afterRender=function()
{
  ...
  {
    if(me.firstNode) {
    (me.currentNode=me.firstNode).focus();
    }
...
MzTreeView.prototype.attachEventArrow = function()
{
  ...
  var me= this;  a.attachEvent("onkeydown", function(e)
  {
    e = window.event || e; var key = e.keyCode || e.which;
	 // redstorm 
	 // 增加了 e.returnValue=false; 解决IE中上下键移动时滚动不同步
    switch(key)
    {
      case 37 : me.focusUpperNode();e.returnValue=false;       break;  //Arrow left,  shrink child node
      case 38 : me.focusPreviousSibling();e.returnValue=false; break;  //Arrow up
      case 39 : me.focusLowerNode();e.returnValue=false;       break;  //Arrow right, expand child node
      case 40 : me.focusNextSibling();e.returnValue=false;     break;  //Arrow down
  ...
  switch(e.tagName)
  {
    ...
    case "A":
      if(B){ n.focus(); 
		...
		try {
			//treeNodeClick(n);
			Dictionary.treeNodeClick(n,this.useCheckbox);//节点单击事件
		}
		catch (err) {}
		/*
		var nodeMethod=n.get("method");
		if (("undefined" != typeof nodeMethod) && (nodeMethod != "")) {
		  eval(nodeMethod);
		}
		*/
...
MzTreeView.prototype.focus = function(id) {
   ...
   if (this.isExistNode(id) == 1) { // 增加结点是否存在判断
...
MzTreeView.prototype.focusNodeByPath = function(path) {
   ...
   if ((n=path.indexOf(this.divider))>0) {
      if (this.isExistNode(path.substring(0, n)) == 1) { // 增加结点是否存在判断
         var node = this.getNodeById(path.substring(0, n));
         if (node!=this.rootNode && !node.expanded) {
            node.expand();
         }
         node.focus();
      ...
      if (this.isExistNode(path) == 1) { // 增加结点是否存在判断
         this.getNodeById(path).focus();
      }
   }
};
//*
...
//  alert(icon);
  ...
  if(!icon || typeof(MzTreeView.icons.collapse[icon])=="undefined"|| icon==""|| icon==null)
  ...
  if (!isRoot)
     s += "<img border='0' id='"+ $.index +"_expand_"+ id +"' src='"+
       ...
       MzTreeView.icons.line[this.line].src)+"'>"; 
  if ($.showNodeIcon)
  ...
  s += "</td><td style='padding-left:3' nowrap='true' valign=bottom><a href='"+ url +
...
MzTreeNode.prototype.check = function(checked)
{
  ...
  if(checked){//选中子节点 触发事件
	  Dictionary.treeNodeChecked(me);
  }else{//取消选中
	  Dictionary.treeNodeUnChecked(me);
  }
};
...
//取消焦点
MzTreeNode.prototype.unfocus=function()
{
  var $ = this.$$caller, a=$.rootNode, o;
  if(!$.selectedNode) $.selectedNode=a;
  if(a = document.getElementById($.index +"_link_"+ this.index)){
...
MzTreeNode.prototype.appendNode=function(node)
{
  ...
  if(div.insertAdjacentHTML) div.insertAdjacentHTML("beforeEnd",node.renderAppend(true));  // 原来语句 node.render(true);
...
// 增加结点生成HTML（新增方法）
MzTreeNode.prototype.renderAppend = function(last) {
  var $=this.$$caller, s=$.dataSource[this.sourceIndex],target,data,url;
  var icon=s.getAttribute("icon");
  if (!(target=s.getAttribute("target")))
     target=$.getDefaultTarget();
  var hint=$.showToolTip ? s.getAttribute("hint") || this.text : "";
  if (!(url=s.getAttribute("url"))) 
     url = $.getDefaultUrl();
  if (data=s.getAttribute("data"))
    url+=(url.indexOf("?")==-1?"?":"&")+data;
  var id=this.index, s="";
  var isRoot=this.parentNode==$.rootNode;
  if (isRoot && $.convertRootIcon && !icon) 
     icon = "root";
  if (!isRoot)
     this.childPrefix=this.parentNode.childPrefix+(last?",ll":",l4");
  ...
  s += "<div><table border='0' cellpadding='0' cellspacing='0'>"+
       "<tr title='"+ hint +"'><td id='"+ $.index + "_line_"+ id +"'>"; // 增加ID
    // 新增代码开始
  var brotherNodeLine = "";
  if (this.parentNode.hasChild && this.parentNode.childNodes.length > 1) {
     var brotherNode = this.parentNode.childNodes[this.parentNode.childNodes.length-2];
	  var brotherNodeLineTd = document.getElementById($.index +"_line_"+ brotherNode.index);
	  if (brotherNodeLineTd != null) {
	     brotherNodeLine = brotherNodeLineTd.innerHTML;
	  }
  }
  // 新增代码结尾
  if (MzTreeNode.htmlChildPrefix)
     s += brotherNodeLine + "</td><td>";  
     // s += MzTreeNode.htmlChildPrefix +"</td><td>";  // 原语句,增加结点时不对
  if (!isRoot)
     s += "<img border='0' id='"+ $.index +"_expand_"+ id +"' src='"+
       (this.hasChild ? MzTreeView.icons.collapse[this.line].src : 
       MzTreeView.icons.line[this.line].src)+"'>"; 
  ...
  if ($.useCheckbox)
     s += "<img border='0' id='"+$.index +"_checkbox_"+ id +"' src='"+ 
          MzTreeView.icons.line["c"+ (this.checked?1:0)].src +"'>";
  s += "</td><td class='nodeStyle' nowrap='true' valign=bottom><a href='"+ url +
       "' target='"+ target +"' id='"+$.index +"_link_"+ id +
       "' class='MzTreeView'>"+ this.text +"</a></td></tr></table><div ";
  if (isRoot&&this.text=="") 
     s="<div><div ";
