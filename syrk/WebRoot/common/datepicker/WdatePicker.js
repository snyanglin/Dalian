...
(function () {
   ...
   function K(H, $) {
      ...
      if (!$dp.dd || $dp.eCont || ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang && $dp.getLangIndex && $dp.getLangIndex($dp.lang) >= 0)) {
         ...
         if (_.$dpPath == "") F(A);
         var B = "<iframe id=\"dateFrame\" style=\"width:1px;height:1px\" src=\"" + _.$dpPath + "My97DatePicker.htm\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
         if ($dp.eCont) {
         ...
         // 修改只读不出现选择 开始--->
         try {
            var isReadonly = $dp.el.getAttribute("readOnly");
            if ((IE && isReadonly) || (!IE && isReadonly != null)) {
            	return;
            }
         }
         catch (err){}
