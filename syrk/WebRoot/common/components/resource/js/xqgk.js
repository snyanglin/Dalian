...
//			_MapApp.moveMapToBjzbz(bjzbz);
//		}
		
		//统计辖区内概况
		this.showXqgk(cacheOrgCode);
...
//	/**
//	 * markPoints
//	 * @param jsonlist 单位地址描述信息
//	 * @param callback
//	 * @returns
//	 */
//	this.createMarker = function (jsonlist, callback) {
//		var image = "";
...
//	};
	...
	this.showXqgk = function(zzjgdm) {
		
		//维护页面的组织机构区域码缓存
		$("#cache_orgCode").val(zzjgdm);
		
		xqgk.showOrgArea(zzjgdm);
		
		SysTool.ajax({
			url : '/syrk/xqgk/getData',
			data : {
				'orgCode' : zzjgdm 
			}
		}, function(json) {
			//设置页面上的统计结果
			$("#val_syrkTotal").text(json["syrkTotal"]);
			$("#val_czrkCount").text(json["czrkCount"]);
			$("#val_jzrkCount").text(json["jzrkCount"]);
			$("#val_ldrkCount").text(json["ldrkCount"]);
			$("#val_wlhrkCount").text(json["wlhrkCount"]);
			$("#val_jwryCount").text(json["jwryCount"]);
			$("#val_sydwTotal").text(json["sydwTotal"]);
			$("#val_ldyCount").text(json["ldyCount"]);
			$("#val_tzhyCount").text(json["tzhyCount"]);
			$("#val_ylcsCount").text(json["ylcsCount"]);
			$("#val_qthyCount").text(json["qthyCount"]);
			$("#val_syfwTotal").text(json["syfwTotal"]);
			$("#val_syfwCount").text(json["syfwTotal"]);
			$("#val_czfwCount").text(json["czfwCount"]);
			$("#val_bzdzTotal").text(json["bzdzCount"]);
			$("#val_bzdzCount").text(json["bzdzCount"]);
		});
...
//			//警力列表
			...
			$("#results").append(
					"<a  href='javascript:xqgk.clickFunc(\""+resultSet[j].SBID+"\")' id='"+resultSet[j].SBID+"'>" + "<span class='span1'>" + (resultSet[j].JYXM||"空") + "</span>"
...
//			}
	...
	this.showOrgArea = function(pnodes){
				...
				json.MMM = setInterval(function(){
					if(i < countZrq){
			        	if(undefined != json[i].bjzbz && null!=json[i].bjzbz && json[i].bjzbz!=""){
			        		var col = colors[i];
			        		xqgk.showAreas(json[i].zzjgdm, json[i].zzjgmc, json[i].bjzbz, col);
			        	}
		...
		for ( var i = 0; i < resultSet.length; i++) {
			...
			if (allcount == 0) {
				...
				for ( var j = 0; j < jsonlist.length; j++) {
					...
					$("#results").append(
							"<a href='javascript:xqgk.clickFunc(\""+jsonlist[j].SBID+"\")' id='"+jsonlist[j].SBID+"'>" + "<span class='span1'>" + (jsonlist[j].JYXM||"空") + "</span>"
...
//					}
	...
	function createMarker(jsonlist, callback) {
		...
		var markerParam = {markerArray:markerArray,
					contentType:"html",
					contentValue:initBubbleContent(),
					contentCallback:"xqgk.loadBubbleContent",
					eventFuncJson:{},
					bubbleShowEvent:"click",
					titleShowEvent:"",
					bubbleWidth:400,
	...
	function initBubbleContent(){
		...
		msg += "<a class='btn_tipsdw_s' id='gpsbubble_pj' href='javascript:xqgk.bubblePjFunc(0)'  style='display:none;'>派警</a>";
		msg += "<a class='btn_tipsdw_s' id='gpsbubble_zy' href='javascript:xqgk.bubblePjFunc(1)'  style='display:none;'>增援</a>";
		msg += "<a class='btn_tipsdw_s' id='gpsbubble_gp' href='javascript:xqgk.bubblePjFunc(2)'  style='display:none;'>改派</a>";
	...
	function loadBubbleContent(data){
		...
		function loadGpsInfoDetail(data) {
			...
			//绑定主监控事件
			$("#gpsbubble_zjk").attr("href","javascript:xqgk.gpsZjk('"+data["SBID"]+"')");
			//绑定轨迹事件
			$("#gpsbubble_lsgj").attr("href","javascript:xqgk.addGpsLsgj('"+data["SBID"]+"')");
		...
		function loadGpsPersonQwInfo(data) {
			...
			$("#gpsbubble_xlqy").attr(
					"href",
					"javascript:xqgk.addGpsXlqy('" + data["XQLX"]
							+ "','" + data["XQZB"] + "')");
		}
	}
