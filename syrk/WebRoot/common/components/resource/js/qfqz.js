...
function Qfqz() {
	 ...
	 * 页面初始化
	 */
	this.init = function(zzjgdm) {
		
		var cacheOrgCode = qfqz.getCacheOrgCode();
		if(!cacheOrgCode){
			cacheOrgCode = zzjgdm;
		}
		...
		var mapLevel = _MapApp.getZoomLevel();
		if(mapLevel<8){
			_MapApp.centerAndZoom(new Point("121.59619","38.90576"),12);
		}
	};
	
	this.getCacheOrgCode = function(){
		return $("#cache_orgCode").val();
	};
	
	this.setChageOrgCode = function (selectedOrgCode){
		$("#cache_orgCode").val(selectedOrgCode);
	}
	
	this.loadXlqy = function (data){
		//关闭弹窗
		_MapApp.closeInfoWindow();
		
		clearXlqy();
		showQfqzArea(data);
		createQfqzMaker(data);
	};
	
	this.centerToXlqy = function(bjzbz){
		
		if(bjzbz && bjzbz != "" && bjzbz.length > 10){
			var line = new Polyline(bjzbz, "red", 3, 0.5,0);
			var pMbr = line.getMBR();
//			var ObjMap = new Polygon(bjzbz, "blue", 3, 0.5,"#330OFF");
//			var pMbr = ObjMap.getMBR();
	...
	this.centerToXq = function(orgCode){
		SysTool.ajax({
			...
			if(json!=null&&json.length>0){
				var ObjMap = new Polygon(json[0].bjzbz, "blue", 3, 0.5,"#330OFF");
				var pMbr = ObjMap.getMBR();
				_MapApp.centerAtMBR(pMbr);
			}
		});
	...
	function clearXlqy(){
		...
		try{
			for(var item in olArray){
				var olObj = olArray[item];
				_MapApp.removeOverlay(olObj); 
			}
		}catch(e){}
	};
	
	function showQfqzArea(data){
		
		for(var i = 0; i < data.length; i++){
			//长度不足10位的明显是垃圾数据
			if(data[i].xlqy && data[i].xlqy != "" && data[i].xlqy.length > 10){
				
				var line = new Polyline(data[i].xlqy, "red", 3, 0.5,0);
				line.addListener("click", function(){
					alert(i);
				});
				var pMbr = line.getMBR();
				
				//var ObjMap = new Polygon(data[i].xlqy, "blue", 3, 0.5,"#cbd");
				//var pMbr = ObjMap.getMBR();
...
//					_MapApp.addOverlay(ObjMap);
					_MapApp.addOverlay(line);
				} catch (e) { }
			}
		}
	}
	
	function createQfqzMaker(data){
		
		var markerArray = new Array();
		for(var i = 0;i < data.length ; i++){
			var imgSrc = qfqz.qfqz_img[i];
		...
		var markerParam = {
			...
			contentType:"html",
			contentValue:initQfqzBubbleContent(),
			contentCallback:"qfqz.loadQfqzBubbleContent",
			eventFuncJson:{},
		...
		MapBubble.addMarkerToMap(markerParam);
	}
	
	function initQfqzBubbleContent (){
		var mapContent = "";
		mapContent += "<div id='ssjl_gps' value='' class='tips_top'>";
		mapContent += "<div class='tips_title'><strong style='padding:0 10px;'>群防群治</strong>";
		mapContent += "</div>";
		mapContent += "</div>";
		mapContent += "<div class='tips_jlct'>";
		mapContent += "<div class='tipsjili'>";
		mapContent += "组织名称：<span style='color:#000' id='zzmc'></span>";
		mapContent += "</div>";
		mapContent += "<div class='tipsjili'>";
		mapContent += "类型：<span style='color:#000' id='zzlbdm' title=''></span>";
		mapContent += "</div>";
		mapContent += "<div class='tipsjili'>";
		mapContent += "组织形式：<span style='color:#000' id='ffzzxsdm'></span>";
		mapContent += "</div>";
		mapContent += "<div class='tipsjili'>";
		mapContent += "联系方式：<span style='color:#000' id='zzlxfs'></span>";
		mapContent += "</div>";
		mapContent += "</div>";
		return mapContent;
	};
	
	this.loadQfqzBubbleContent = function (data){
		
		var ffzzxsdm = window.top.getDictName(path+'/common/dict/D_ZAFF_ZZXS.js', data["ffzzxsdm"]);
		var zzlbdm = window.top.getDictName(path+'/common/dict/BD_D_ZZLB.js', data["zzlbdm"]);
		
		$("#zzmc").text(data["zzmc"]|| "未知");
		$("#zzlbdm").text(data["zzlbdm"] || "未知");
		$("#ffzzxsdm").text(ffzzxsdm || "未知");
		$("#zzlbdm").text(zzlbdm || "未知");
	};
...
//					XjMap.bjJson = json;
	...
	this.qfqz_colors = ["#66CC00","#330OFF","#FF99CC","#66CCCC","#9966CC","#CC9999","#FFCC00","#CCO00O"];
	
	this.qfqz_img = [
	    "/syrk/common/components/resource/images/jtts/jq00.png",
	    "/syrk/common/components/resource/images/jtts/jq01.png",
	    "/syrk/common/components/resource/images/jtts/jq02.png",
	    "/syrk/common/components/resource/images/jtts/jq03.png",
	    "/syrk/common/components/resource/images/jtts/jq04.png",
	    "/syrk/common/components/resource/images/jtts/jq05.png",
	    "/syrk/common/components/resource/images/jtts/jq06.png",
	    "/syrk/common/components/resource/images/jtts/jq07.png",
	    "/syrk/common/components/resource/images/jtts/jq08.png",
	    "/syrk/common/components/resource/images/jtts/jq09.png"
	];
