...
HomePage.intoMap = function(pnodes){
	...
	if(pnodes!=''){
		...
		var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryXjZzjgxj",HomePage.queryXjZzjg_back);
		fajax.send(params);
	}
};
...
HomePage.queryXjZrq = function(pnodes){ 
	if(pnodes!=''){
		...
		var fajax =new FrameTools.Ajax(contextPath+"/homePage/downMap", HomePage.intoMap);
		fajax.send(params);
	}
};
...
HomePage.syrkListMBR = function(lx){
	if(typeof EzServerClient != "undefined"){
		...
		var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryXjList",HomePage.syrkListMBR_back);
		fajax.send(params);
	}
};
/**
