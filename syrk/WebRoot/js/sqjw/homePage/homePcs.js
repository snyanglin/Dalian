...
HomePcs.queryXjZzjg = function(){
	...
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryXjZzjg",HomePcs.queryXjZzjg_back);
	fajax.send(params);
};
/**
...
HomePcs.queryXjZzjg_back = function(json){
	...
	if(json!=null&&json.length>0){
		...
		var setInt = setInterval(function(){
			if(count<HomePcs.countZrq){
				if(json[count].bjzbz!=""&&json[count].bjzbz!=null){
	        		var col = colors[count];
	        		HomePcs.showArea(json[count].zzjgdm,json[count].zzjgmc,json[count].bjzbz,col);
	        	}
			}else{
...
HomePcs.queryZrqUser = function(){
	...
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryZrqUser",HomePcs.queryZrqUser_back);
	fajax.send(params);
};
/**
...
HomePcs.initFloatWin = function(zzjgdm){
	...
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryPcsTj",HomePcs.initFloatWin_back);
	fajax.send(params);
};
/**
...
HomePcs.jwtsOpen = function(){
	var url = "http://10.80.8.178:9080/Component";
...
HomePcs.zdryListMBR = function(lx){
	...
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryZdrkMap",HomePcs.zdryListMBR_back);
	fajax.send(params);
};
/**
...
HomePcs.syfwListMap = function(lx){
	...
	$(document.getElementById("tdmapid")).mask("数据加载中...");
	var params = {lx:lx,zzjgdm:HomePcs.zzjgdm};
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/querySyfwMap",HomePcs.syfwListMap_back);
	fajax.send(params);
};
/**
...
HomePcs.sydwListMap = function(lx){
	...
	$(document.getElementById("tdmapid")).mask("数据加载中...");
	var params = {lx:lx,zzjgdm:HomePcs.zzjgdm};
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/querySydwMap",HomePcs.sydwListMap_back);
	fajax.send(params);
};
/**
...
HomePage.syrkListMBR = function(lx){
	HomePcs.removeMap();
	$(document.getElementById("tdmapid")).mask("数据加载中...");
	var params = {syrkywlxdm:lx,gxzrqdm:HomePcs.zzjgdm};
...
HomePage.syrkListMBR_back = function(json){
	...
	var listLen = json.length;
	for(var i=0;i<listLen;i++){
		HomePcs.openCh(json[i].xm,json[i].jzd_zbx,json[i].jzd_zby);
	}
	$(document.getElementById("tdmapid")).unmask();
};
/**
 ...
 */	
HomePcs.openCh = function(mc,zbx,zby){
	var initMarker = HomePcs.map.initMarker(mc,zbx,zby,"syrkBlue.png",null,null,43,37);
	HomePcs.map._MapApp.addOverlay(initMarker);
	HomePcs.mpdzMarkerArr.push(initMarker);
};
/**
...
HomePage.twspMap = function(){
	...
	$(document.getElementById("tdmapid")).mask("数据加载中...");
	var params = {zzjgdm:HomePcs.zzjgdm};
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/querytwspMap",HomePage.twspMap_back);
	fajax.send(params);
};
/**
 * @title:twspMap_back
...
HomePcs.queryPcs = function(){
	var params = {};
	var fajax =new FrameTools.Ajax(contextPath+"/homePage/queryPcs",HomePcs.queryPcs_back);
	fajax.send(params);
};
/**
 * @title:queryPcs_back
