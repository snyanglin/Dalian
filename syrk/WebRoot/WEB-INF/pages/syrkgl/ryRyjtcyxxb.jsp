...
<body>
	<form action="" id="dataForm" name="dataForm" method="post" >
		...
		<input type="hidden" id="jtcyryid" name="jtcyryid" value="${entity.jtcyryid}"/>
		...
		<table border="0" cellpadding="0" cellspacing="10" width="846" align="left">
		    ...
		    <tr class="dialogTr">
		    	...
		    	<input type='hidden' name='fwcsid' id="fwcsid" value="${entity.fwcsid}"/>
		    	<input type='hidden' name='fwcs' id="fwcs" value="${entity.fwcs}"/>
		    	<input class="easyui-combobox" name="fwcs-box" id="fwcs-box" value="${entity.fwcs}" data-options="mode:'remote',method:'post',panelHeight: 22,valueField:'id',textField:'text',selectOnNavigation:false,required:false" type="text" maxlength="50"  style="width:618px;" /></td>
	    	</tr>
	    	<tr class="dialogTr">
...
function beforeSubmit() {
	...
	if ($("#pk").val() == "") {
		...
		$.ajax({
			type:"GET",
			url:"<%= basePath%>ryRyjtcyxxb/queryCount",
			dataType:"json",
			data:"ryid="+$("#ryid").val()+"&jtcyryid="+$("#jtcyryid").val(),
			async:false,
			success:function(data){
				if(data && data != null && data != undefined && data == 1){
					$.messager.alert('确认对话框', '该家庭成员已存在！请重新输入', 'warning', function(){
						$("#zhhm").focus();
					});
					_count = 1;
				}
		...
		$("#_method").val('');
		$("#dataForm").attr('action', contextPath + '/ryRyjtcyxxb/save');
	}
	else {
		$("#_method").val('PUT');
		$("#dataForm").attr('action', contextPath + '/ryRyjtcyxxb/' + $("#pk").val());
	}
	...
	$.ajax({
		...
		success: function(data) {
			if (data && data.ryRyjbxxb) {
				$("#jtcyryid").val(data.ryRyjbxxb.id);
...
$(document).ready(function(){
	...
	initDepartmentSearch('fwcs-box', {glpcsid: ''}, 'fwcsid', 'fwcs', {});
	//复用人员基本表
