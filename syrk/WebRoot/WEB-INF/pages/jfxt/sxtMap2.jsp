...
<script type="text/javascript">
	...
	function addCameraMark(){
			...
			var url="<%=contextPath%>/sptgl/querySpsxtForMap";
			var params={"add":"add"};//参数add有值表示通过地图获取坐标"add":"add"，否则为修改坐标
			//返回json格式的字符串
	...
	function drawNewPoint(idx){
			...
			_MapApp.changeDragMode('drawPoint',null,null,function(result){
				...
				//移除该行的click事件
				rowObject.hide();//styleunbind('click');//隐藏行(该行已被修改)
...
function bandEventForRow(){
		...
		$("#infoTable tr").live('click',function(){
			...
			drawNewPoint(curRowIndex);//绘制新坐标	
			/*					
