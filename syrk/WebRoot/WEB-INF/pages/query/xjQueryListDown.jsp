    ...
    <div data-options="region:'center',border:false">
    	<div class="easyui-layout" data-options="fit:true,border:false">   
            <div data-options="region:'center',split:true,title:'',border:false" >
            	...
            	data-options="
            	queryParams: {'zdrylxdm': getParm('zdrylxdm'), 'xm': getParm('xm'), 'gmsfhm': getParm('gmsfhm'), 'gxfxjdm': getParm('gxfxjdm'), 'gxpcsdm': getParm('gxpcsdm'), 'gxzrqdm': getParm('gxzrqdm')},
            	url: '<%=basePath %>xj/queryZdryjbxxNoPt',
            	delayCountUrl:'<%=basePath %>xj/queryCountZdry',
            	toolbar:'#datagridToolbar',
            		...
            		sortOrder:'desc',
            		idField:'id',
            		pageSize:getAutoPageSize(670),
            		pageList:[getAutoPageSize(670),getAutoPageSize(670) * 2],
				    ...
				    <thead>
				        <tr>
				            ...
				            <th  data-options="field:'dz_jzdzxz',width:200,align:'left',halign:'center',sortable:true,formatter:subjzdz">居住地址</th>
		...
		for (var i = 0; i < len; i++) {
			...
			if (dz != "" && rows[i].zbx != null && rows[i].zbx != "") {
				if (option.indexOf('zdrk') != -1) {
					var marker = null;
					if (IE) {
						marker = mapWindow.addMarker('',rows[i].zbx,rows[i].zby,"zdrk.png",null,null,26,26,false,rows[i].dz_jzdzdm, 'zdrk');
					}
					else {
						marker = mapWindow.addMarker('',rows[i].zbx,rows[i].zby,"zdrk.png",null,null,30,30,false,rows[i].dz_jzdzdm, 'zdrk');
					}
