...
<html>
  ...
  <body class="easyui-layout" data-options="fit:true">
    	...
    	<div class="easyui-layout" data-options="fit:true,border:false">   
            <div data-options="region:'center',split:true,title:'受理咨询求助信息',border:true" style="height:auto">
			<table id="dg" class="easyui-datagrid" data-options="url:'<%=basePath %>fwqz/querySlzxqz',toolbar:'#datagridToolbar',
			            singleSelect:false,
			            selectOnCheck:true,
			            checkOnSelect:true,
			            border:false,
			            ...
			            pageList:[getAutoPageSize(175),getAutoPageSize(175) * 2]">
							    <thead>
							        <tr>
							            <th data-options="field:'zxqzsj', 	width:10,	align:'center',	halign:'center',sortable:true">咨询求助时间</th>
							            <th data-options="field:'zxqznr', 	width:30,	align:'center',	halign:'center',sortable:true">咨询求助内容</th>
							            <th data-options="field:'jdjjfs', 	width:20,	align:'center',	halign:'center',sortable:true">解答解决方式</th>
					...
					<form id="queryForm">
						<table cellspacing="0" cellpadding="0" border="0" >
							...
							<tr class="dialogTr">
								<td class="toolbarTd">
									咨询求助内容：<input type="text" name="zxqznr" id ="zxqznr" class="easyui-validatebox" 
									 		data-options="required:false,validType:'maxLength[20]'"  
											 />
								</td>
								<td class="toolbarTd"><div class="datagrid-btn-separator"></div></td>
								<td class="toolbarTd">
									解答解决方式：<input type="text" name="jdjjfs" id ="jdjjfs" class="easyui-validatebox" 
									 		data-options="required:false,validType:'maxLength[20]'"  
											 />
								</td>
								<td class="toolbarTd"><div class="datagrid-btn-separator"></div></td>
								<td class="toolbarTd">
										咨询求助时间起：<input type="text" name="startDate" id="startDate" class="easyui-validatebox " 
								...
								<td class="toolbarTd"><div class="datagrid-btn-separator"></div></td>
								<td class="toolbarTd">
										咨询求助时间止：<input type="text" name="endDate" id="endDate" class="easyui-validatebox " 
														data-options="validType:['date[\'yyyy-MM-dd\']'],tipPosition:'right'" 
...
function info(linkObject, index){
	...
	var hsUrl = "/fwqz/slzxqzInFo?";
	hsUrl+="id="+rowData.id+"&mainTabID="+getMainTabID();
	menu_open("受理咨询求助信息",hsUrl);
}
//操作列
...
function queryButton(){
  	var zxqznr = document.getElementById("zxqznr").value;
  	var jdjjfs = document.getElementById("jdjjfs").value;
...
function doEdit(linkObject, index){
		...
		hsUrl+="id="+rowData.id+"&mainTabID="+getMainTabID();
		menu_open("受理咨询求助信息编辑" ,hsUrl);
  }
  
//注销单条
  function doDelete(linkObject, index) {
	  cancelBubble(); // 阻止冒泡，不然要执行onClickRow
		var deleteUrl = contextPath + '/fwqz/deleteSlzxqzb';
		var datagrid_ID = getDatagrid_ID(0, linkObject);
...
function dbClickRow(rowIndex,rowData){
	var hsUrl = "/fwqz/slzxqzInFo?";
		hsUrl+="id="+rowData.id+"&mainTabID="+getMainTabID();
		menu_open("受理咨询求助信息" + rowData.xm,hsUrl);
}

function add(){
	var hsUrl = "/fwqz/addSlzxqz?";
	hsUrl+="&mainTabID="+getMainTabID();
	menu_open("受理咨询求助信息新增",hsUrl);
}
