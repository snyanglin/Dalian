...
function doInit(paramArray) {
	...
	document.getElementById("descTd").innerHTML = "说明：双击人员可以选择到右边列表，"+ desc;
	...
	$('#treeSelect').tree({
		...
		onDblClick: function(node) { // 在双击一个节点的时候执行
			...
			if (nodeType == "user") { // 根结点不变
				...
				addOptions(optionValue, node['text'], node['bizID'], parentNode['bizID'], parentNode['text'], parentNode['id']);
			}
		}
...
function ok_execute() {
	...
	if (userIDInputID) {
		parentWindow.document.getElementById(userIDInputID).value = returnBizIDArray.join(",");
		parentWindow.$('#' + userIDInputID).validatebox('validate');
	}
	if (userNameInputID) {
		parentWindow.document.getElementById(userNameInputID).value = returnNameArray.join(",");
		parentWindow.$('#' + userNameInputID).validatebox('validate');
	}
	if (userTableIDInputID) {
		parentWindow.document.getElementById(userTableIDInputID).value = returnIDArray.join(",");
...
function button2_onclick() {
	...
	for (var item in nodes) {
		...
		if (nodeType == "user") {
			var nodeID = node['id'];
			var parentID = nodeID.substring(0, nodeID.indexOf("_"));
			var parentNode = treeObject.tree('find', parentID);
			var optionArray = [];
...
function getOptionString(optionValue, optionText, bizID, orgCode, orgName, orgID) {
	...
	optionString += '\xA0' + bizID + '\xA0|\xA0' + optionText + '</options>';
	return optionString;
}
