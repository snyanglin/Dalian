    ...
    <div data-options="region:'center',border:false">
    	<div class="easyui-layout" data-options="fit:true,border:false">   
            <div data-options="region:'center',split:true,title:'岗位列表',border:false" style="height:auto">
            	<table id="dg" class="easyui-datagrid" data-options="url: contextPath +'/orgRole/queryList',toolbar:'#datagridToolbar',
				    ...
				    <thead>
				        <tr>
				            ...
				            <th data-options="field:'rolename',width:20,align:'left',halign:'center',sortable:true">角色名称</th>
				            <th data-options="field:'roledesc',width:30,align:'left',halign:'center',sortable:true">角色描述</th>
				            <th data-options="field:'orderno',width:10,align:'center',halign:'center',sortable:true">角色顺序号</th>
					...
					<tr>
						<td class="toolbarTd">
					    	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-query" plain="true" onclick="datagridQuery(this, 'queryWindow', 
					    		...
					    		{
					    		title: '组织机构－角色查询条件',
					    		url: contextPath + '/orgRole/queryCondition',
					    		width: 850,
					    		height: 320
					    		})">查询</a>
						</td>
						...
						<td class="toolbarTd">
					    	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="datagridDeletePatch(this, '', 
					    		'',
					    		{
					    		url: contextPath + '/orgRole/deletePatch'
					    		})">批量删除</a>
						</td>
					</tr>
					</tbody>
...
function queryMinMaxOrder() {
	$.ajax({
		url: contextPath + '/orgRole/queryMinMaxOrder',
		dataType:"json",
		type: 'POST'
...
function moveUp(linkObject, rowIndex) {
	...
	if (orderNO == minOrderNO) {
		...
		if (rowIndex == 0) { // 本页的第一行
			...
			$.ajax({
				url: contextPath + '/orgRole/moveSave',
				type: 'POST',
				data: postData
...
function doOnClickRow(rowIndex, rowData) {
	var editUrl = contextPath + '/orgRole/edit';
...
function doDelete(linkObject, index) {
	cancelBubble(); // 阻止冒泡，不然要执行onClickRow
	var deleteUrl = contextPath + '/orgRole/delete';
	var datagrid_ID = getDatagrid_ID(0, linkObject);
